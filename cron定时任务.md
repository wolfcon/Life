...
## cron 一共有3个配置
### 1. /var/spool/cron/ 
这个`目录`下存放的是每个用户包括 `root` 的 `crontab` 任务, 每个任务以创建者的名字命名, 比如 `root` 建的 crontab 任务对应的文件就是 `/var/spool/cron/root` .一般一个用户最多只有一个 crontab 文件.
...
### 每个 `*` 号都是可以用`数字`代替, 並且有多个选项则用 `,` 隔开即可.
